...
#Если токенизатор и модель уже были загружены, они извлекаются из кэша моделей вместо повторной загрузки. Нерационально, потому-что съедает слишком много ОЗУ
...
def clear_gpu_cache():#Функция torch.cuda.empty_cache() используется для очистки кэша памяти на GPU.
